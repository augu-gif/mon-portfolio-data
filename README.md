# Portfolio 

## À propos de moi
Je m'appelle Augustin Montreuil, j'ai 30 ans et je souhaite me reconvertir dans le domaine de la data. Grâce à ce portfolio, je vais pouvoir partager mes différentes avancées dans mon projet de reconversion.  

## Mes projets
- [Mon premier site web](https://github.com/augu-gif/mon-premier-site-web/blob/main/README.md) : Première approche avec le monde du numérique
- [Analyse des données Spotify]( https://github.com/augu-gif/projet-spotify/blob/main/README.md) : Classification et analyse des préférences musicales d'un utilisateur.
- [Projet Extraction Automatique de Numéros SIREN](https://github.com/augu-gif/Projet_Numero_de_SIREN/blob/main/README.md) : Utilisation de l'IA pour trouver le numéro de SIREN dans un fichier d'annonces légales
- [Projet crise du logement](https://github.com/augu-gif/Projet_crise_logement/tree/portfolio?tab=readme-ov-file) : Ce projet en équipe qui cloture la formation de deux mois au Wagon analyse la crise du logement dans la Métropole Européenne de Lille et propose une solution concrètes et data-driven pour réduire le nombre de personnes sans domicile grâce à des leviers fiscaux.
## Contact
- Email : augustin.mont@gmail.com
- LinkedIn : https://www.linkedin.com/in/augustin-montreuil-b00aa1281/

